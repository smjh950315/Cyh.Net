...
#pragma warning restore CS8602
        ...
        /// <param name="filter">組件過濾器</param>
        /// <returns></returns>
        public static IEnumerable<Assembly> GetAllReferenceModuleAssemblies(this Assembly assembly, Func<AssemblyName, bool>? filter = null)
        {
            List<Assembly> result = [];
            static void FindAndAddReferenceAssembly(Assembly _asm, ref List<Assembly> assemblies, Func<AssemblyName, bool>? filter = null)
            {
                IEnumerable<AssemblyName> assemblyNames = _asm.GetReferencedAssemblies();
                if (filter != null)
                {
                    assemblyNames = assemblyNames.Where(x => filter(x));
                }
                foreach (AssemblyName assemblyName in assemblyNames)
                {
                    ...
                    if (referencedAssembly != null)
                    {
                        ...
                        FindAndAddReferenceAssembly(referencedAssembly, ref assemblies, filter);
                    }
                }
            }
            FindAndAddReferenceAssembly(assembly, ref result, filter);
            return result;
        }
        ...
        /// <param name="pattern">組件名稱(模糊)</param>
        /// <param name="assembly">組件</param>
        public static void AssertAssemblyNotNull(this IEnumerable<Assembly> assemblies, string pattern, [NotNull] out Assembly? assembly)
        {
            Assembly? module = assemblies.FirstOrDefault(x => !x.FullName.IsNullOrEmpty() && x.FullName.Contains(pattern));
            Debug.Assert(module != null, $"找不到組件 {pattern}");
            assembly = module;
        }
    }
}
